...
export class SerenityThermostatCard extends HTMLElement {
  ...
  _css() {
    return `
      ...
      ha-card { padding: 18px 20px 20px; border-radius: var(--ha-card-border-radius, 22px); }
      ...
      .icon-box ha-icon { --mdc-icon-size: 22px; color: var(--_accent); }
      .htext { min-width: 0; }
      .name { font-size: 18px; font-weight: 700; line-height: 1.2; color: var(--_value); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
      .sub { font-size: 13.5px; font-weight: 500; color: var(--_muted); margin-top: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
      .sub.hidden { display: none; }
      .iconbtn {
        flex: 0 0 auto; width: 42px; height: 42px; border: none; padding: 0; cursor: pointer;
        border-radius: 13px; background: var(--_soft); color: var(--_accent);
        display: flex; align-items: center; justify-content: center;
        transition: background 0.2s ease, color 0.2s ease, transform 0.05s ease;
      }
      .iconbtn ha-icon { --mdc-icon-size: 22px; }
      .iconbtn:active { transform: scale(0.94); }
      .iconbtn.off { background: var(--_tile); color: var(--_muted); }

      /* Dial */
      .dial { position: relative; display: flex; align-items: center; justify-content: center; margin: 10px 0 6px; }
      .gauge { width: 100%; max-width: 240px; height: auto; display: block; transform: rotate(135deg); }
      .track { fill: none; stroke: var(--_track); stroke-width: 16; stroke-linecap: round; }
      .prog { fill: none; stroke: var(--_accent); stroke-width: 16; stroke-linecap: round; transition: stroke-dasharray 0.45s ease, stroke 0.3s ease; }
      .center {
        position: absolute; display: flex; flex-direction: column; align-items: center;
        gap: 9px; transform: translateY(-6px); pointer-events: none;
      }
      .consigne { font-size: 12px; font-weight: 700; letter-spacing: 0.14em; color: var(--_muted); }
      .value { display: flex; align-items: flex-start; color: var(--_value); line-height: 1; }
      .num { font-size: 64px; font-weight: 800; letter-spacing: -2px; }
      .deg { font-size: 26px; font-weight: 700; color: var(--_muted); margin-top: 6px; }
      .pill {
        display: inline-flex; align-items: center; gap: 7px;
        background: var(--_soft); padding: 6px 14px; border-radius: 999px;
      }
      .pill.hidden { display: none; }
      .pdot { width: 7px; height: 7px; border-radius: 50%; background: var(--_accent); }
      .ptext { font-size: 14px; font-weight: 600; color: var(--_accent); }

      /* Steppers */
      .steppers { display: flex; align-items: center; justify-content: center; gap: 16px; margin: 4px 0 18px; }
      .step {
        width: 58px; height: 58px; border: none; padding: 0; cursor: pointer;
        border-radius: 19px; display: flex; align-items: center; justify-content: center;
        transition: background 0.2s ease, transform 0.05s ease;
      }
      .step ha-icon { --mdc-icon-size: 26px; }
      ...
      .tile { background: var(--_tile); border-radius: 16px; padding: 14px 16px; }
      .tlabel { font-size: 13px; font-weight: 500; color: var(--_muted); }
      .tval { margin-top: 4px; color: var(--_value); display: flex; align-items: baseline; }
      .tval .cur-num, .tval .hum-num { font-size: 22px; font-weight: 800; letter-spacing: -0.5px; }
